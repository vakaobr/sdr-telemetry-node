...
- [~] Implementation - In Progress (Phase 2/12)
  ...
  - Phase 7 (radio2 FSM, off-hardware): ✓ Complete — full SDR#2 orchestrator built+tested without hardware: explicit FSM (IDLE/STARTING/RUNNING/STOPPING/FAULTED, watchdog, retries+backoff, SIGKILL escalation, single-owner invariant), ChildProcess runner exercised by scripted fake decoders (run/fail/silent/hang), Skyfield pass predictor (NOAA TLE fixture, offline), schedule blocks + priority decision (override>satellite>block>idle), MQTT publisher w/ LWT + radio2/cmd consumer, supervisor reconciliation loop. Gateway POST /api/v1/radio2/mode (409-during-pass-unless-force). 42 radio2 + 4 gateway-override tests incl. simulated-24h-day, manual-override-expiry, chaos-convergence(≤10 steps), real-subprocess switch. radio2 Dockerfile + node-b compose staged. **NOT deployed — Node B PSU still pending.** 127 tests total.
  - Phase 8 (Node B bring-up + ATC audio): ◐ SOFTWARE COMPLETE, hardware bring-up pending — rtl_airband config generator (single-tuner span validation), atc decoder command, radio2 Dockerfile (compiles rtl_airband v5.0.1), Icecast image + node-b compose, UI AtcPlayer (tap-to-play, channel, squelch activity pulse), gateway override route live on Node A. Runbook: scripts/flash-node-b.md. 163 tests. **BLOCKED on user hardware: (1) Node B PSU swap (still 0x50005), (2) move dongle stx:0:28 to Node B, (3) airband antenna.** Then `docker compose up --build` on Node B → tower audio. Cross-node LWT/audio verification happens at bring-up.
  ...
  - Phase 9 (AIS, M4): ⏳ after Node B is live
